...
class GlassService extends Service implements GlassInterface
{
	...
	public function treatWaste(GlassInterface $waste):void
	{
		...
		if ( $availableCapacity >= $wasteQuantity )
		{
			...
			$waste->setQuantity(0);
			echo $wasteQuantity . " tonnes de déchets ont été traitées (verre). Nouveau remplissage : " . $newUsedCapacity . "/" . $this->getCapacity() . PHP_EOL;
		}
		else
		{
			...
			echo $wasteQuantity - $untreatedWaste . " tonnes de déchets ont été traitées (verre). Il reste " . $untreatedWaste . " tonnes de déchets à traiter. Nouveau remplissage : " . $this->getUsedCapacity() . "/" . $this->getCapacity() . PHP_EOL;
		}
